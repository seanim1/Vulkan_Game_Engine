...
namespace VulkanEngine {
	...
	{
		std::ifstream file(filepath, std::ios::ate | std::ios::binary); // we are at the end of the file 

		if (!file.is_open()) {
			throw std::runtime_error("failed to open file: " + filepath);
		}
		size_t fileSize = static_cast<size_t>(file.tellg()); // tellg() gives us the position, which is at the end of the file, which is file size.
		...
		const PipelineConfigInfo& configInfo)
	{
		...
		shaderStages[0].flags = 0; // not using any
		...
		shaderStages[1].flags = 0; // not using any
